...
public:
	...
	vector<Edge> computeMinimumSpanningTree(const string& start_key, vector<vector<string>>& map_of_graph )
	{
		...
		for (auto edges : accepted_edges)
		{
			map_of_graph.resize(map_of_graph.size() + 1);
			map_of_graph[i].resize(3);

			map_of_graph[i][0] = edges.sink->getKey();
			map_of_graph[i][1] = edges.source->getKey();
			map_of_graph[i][2] = to_string(edges.weight);
